...
@interface QUIModalPresentationViewController ()<QUIKeyboardManagerDelegate>

@property(nonatomic, strong, readwrite) QUIModalPresentationWindow *window;
...
@property(nonatomic, strong) QUIKeyboardManager *keyboardManager;
@property(nonatomic, assign) CGFloat keyboardHeight;
@property(nonatomic, assign) BOOL avoidKeyboardLayout;
@end

@implementation QUIModalPresentationViewController
...
- (void)didInitialize {
    self.animationStyle = QUIModalPresentationAnimationStyleFade;
    ...
    UIViewController *visibleViewController = [VisibleViewControllerHelper visibleViewController];
...
- (void)viewDidLoad {
    ...
    if (self.containsTextField) {
        self.keyboardManager = [[QUIKeyboardManager alloc] initWithDelegate:self];
    }
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.layoutBlock) {
        ...
        self.contentView.frame = CGRectApplyAffineTransformWithAnchorPoint(contentViewFrame, self.contentView.transform, self.contentView.layer.anchorPoint);
    }
}
    ...
    if (animated) {
        ...
        if (self.showingAnimation) {
            ...
            if (self.shouldDimmedAppAutomatically) {
                [UIView animateWithDuration:.25 delay:0 options:QUIViewAnimationOptionsCurveOut animations:^{
...
- (void)viewWillDisappear:(BOOL)animated {
        ...
        if (self.shownInWindowMode) {
            ...
            if (UIApplication.sharedApplication.keyWindow == self.window) {
                if (self.previousKeyWindow.hidden) {
                    // 保护了这个 issue 记录的情况，避免主 window 丢失 keyWindow
    ...
    if (animated) {
        if (self.hidingAnimation) {
            ...
            if (self.shouldDimmedAppAutomatically) {
                [UIView animateWithDuration:.25 delay:0 options:QUIViewAnimationOptionsCurveIn animations:^{
...
- (void)initDefaultDimmingViewWithoutAddToView {
    if (!self.dimmingView) {
        ...
        self.dimmingView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.35f];
        [self addTapGestureRecognizerToDimmingViewIfNeeded];
        if ([self isViewLoaded]) {
            [self.view addSubview:self.dimmingView];
        }
...
- (void)handleDimmingViewTapGestureRecognizer:(UITapGestureRecognizer *)tapGestureRecognizer {
    ...
    if (self.shownInWindowMode) {
        ...
        // 这里仅屏蔽点击遮罩时的 dismiss，如果是代码手动调用 dismiss 的，在 UIViewController(QUIModalPresentationViewController) 里会通过重写 dismiss 方法来屏蔽。
        // 为什么不能统一交给 UIViewController(QUIModalPresentationViewController) 里屏蔽，是因为点击遮罩触发的 dismiss 要调用 willHideByDimmingViewTappedBlock，而 UIViewController 那边不知道此次 dismiss 是否由点击遮罩触发的，所以分开两边写。
...
- (void)setContentViewController:(UIViewController<QUIModalPresentationContentViewControllerProtocol> *)contentViewController {
...
- (void)showingAnimationWithCompletion:(void (^)(BOOL))completion {
    if (self.animationStyle == QUIModalPresentationAnimationStyleFade) {
        self.dimmingView.alpha = 0.0;
        self.contentView.alpha = 0.0;
        [UIView animateWithDuration:.25 delay:0.0 options:QUIViewAnimationOptionsCurveOut animations:^{
    ...
    } else if (self.animationStyle == QUIModalPresentationAnimationStylePopup) {
        self.dimmingView.alpha = 0.0;
    ...
    } else if (self.animationStyle == QUIModalPresentationAnimationStyleSlide) {
        self.dimmingView.alpha = 0.0;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.window) {
        self.window = [[QUIModalPresentationWindow alloc] init];
        self.window.windowLevel = UIWindowLevelAlert - 4.0;
        self.window.backgroundColor = [UIColor clearColor];// 避免横竖屏旋转时出现黑色
    }
...
#pragma mark - <QUIKeyboardManagerDelegate>
- (void)keyboardWillChangeFrameWithUserInfo:(QUIKeyboardUserInfo *)keyboardUserInfo {
...
// 当以 present 方式显示浮层时，状态栏允许由 contentViewController 控制，但 QUIModalPresentationViewController 的 q_prefersStatusBarHiddenBlock/q_preferredStatusBarStyleBlock 优先级会更高
...
@implementation QUIModalPresentationViewController (Manager)

+ (BOOL)isAnyModalPresentationViewControllerVisible {
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        if ([window isKindOfClass:[QUIModalPresentationWindow class]] && !window.hidden) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan {
    ...
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        if (![window isKindOfClass:[QUIModalPresentationWindow class]]) {
            continue;
        }
        ...
        QUIModalPresentationViewController *modalViewController = (QUIModalPresentationViewController *)window.rootViewController;
        BOOL canHide = YES;
        ...
        if (canHide) {
            // 如果某些控件的显隐能力是通过 QUIModalPresentationViewController 实现的，那么隐藏它们时，应该用它们自己的 hide 方法，而不是 QUIModalPresentationViewController 自带的 hideWithAnimated:completion:
            id<QUIModalPresentationComponentProtocol> modalPresentationComponent = nil;
            if ([modalViewController.contentViewController conformsToProtocol:@protocol(QUIModalPresentationComponentProtocol)]) {
                modalPresentationComponent = (id<QUIModalPresentationComponentProtocol>)modalViewController.contentViewController;
            } else if ([modalViewController.contentView conformsToProtocol:@protocol(QUIModalPresentationComponentProtocol)]) {
                modalPresentationComponent = (id<QUIModalPresentationComponentProtocol>)modalViewController.contentView;
            }
...
@implementation QUIModalPresentationWindow
...
- (void)layoutSubviews {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        if (self.rootViewController) {
            ...
            if (CGRectGetMinY(rootView.frame) > 0 && !UIApplication.sharedApplication.statusBarHidden && (UIApplication.sharedApplication.statusBarHidden ? 0 : UIApplication.sharedApplication.statusBarFrame.size.height) > CGRectGetMinY(rootView.frame)) {
