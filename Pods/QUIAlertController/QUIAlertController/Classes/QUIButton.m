...
const CGFloat QUIButtonCornerRadiusAdjustsBounds = -1;
...
- (void)didInitialize {
    ...
    self.imagePosition = QUIButtonImagePositionLeft;
}

// 系统访问 self.imageView 会触发 layout，而私有方法 _imageView 则是简单地访问 imageView，所以在 QUIButton layoutSubviews 里应该用这个方法
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case QUIButtonImagePositionTop:
        case QUIButtonImagePositionBottom: {
            ...
            if (isImageViewShowing) {
                ...
                imageSize.width = MIN(imageSize.width, imageLimitWidth);// QUIButton sizeThatFits 时 self._imageView 为 nil 但 self.imageView 有值，而开启了 Bold Text 时，系统的 self.imageView sizeThatFits 返回值会比没开启 BoldText 时多 1pt（不知道为什么文字加粗与否会影响 imageView...），从而保证开启 Bold Text 后文字依然能完整展示出来，所以这里应该用 self.imageView 而不是 self._imageView
        ...
        case QUIButtonImagePositionLeft:
        case QUIButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 QUIButtonImagePositionLeft 并且titleLabel 多行的情况下，QUIButton 计算的结果与系统不一致
            ...
            if (isImageViewShowing) {
                ...
                imageSize.height = MIN(imageSize.height, imageLimitHeight);// QUIButton sizeThatFits 时 self._imageView 为 nil 但 self.imageView 有值，而开启了 Bold Text 时，系统的 self.imageView sizeThatFits 返回值会比没开启 BoldText 时多 1pt（不知道为什么文字加粗与否会影响 imageView...），从而保证开启 Bold Text 后文字依然能完整展示出来，所以这里应该用 self.imageView 而不是 self._imageView
                imageTotalSize = CGSizeMake(imageSize.width + UIEdgeInsetsGetHorizontalValue(self.imageEdgeInsets), imageSize.height + UIEdgeInsetsGetVerticalValue(self.imageEdgeInsets));
            }
...
- (void)layoutSubviews {
    ...
    if (self.cornerRadius == QUIButtonCornerRadiusAdjustsBounds) {
    ...
    if (isImageViewShowing) {
        ...
        CGSize imageSize = self._q_imageView.image ? [self._q_imageView sizeThatFits:imageLimitSize] : self.currentImage.size;
    ...
    if (isImageViewShowing) {
        makesureBoundsPositive(self._q_imageView);
    }
    if (isTitleLabelShowing) {
        makesureBoundsPositive(self.titleLabel);
    }
    
    if (self.imagePosition == QUIButtonImagePositionTop || self.imagePosition == QUIButtonImagePositionBottom) {
        ...
        if (self.imagePosition == QUIButtonImagePositionTop) {
        ...
        if (isImageViewShowing) {
            self._q_imageView.frame = imageFrame;
        }
        if (isTitleLabelShowing) {
            self.titleLabel.frame = titleFrame;
        }
        
    } else if (self.imagePosition == QUIButtonImagePositionLeft || self.imagePosition == QUIButtonImagePositionRight) {
        ...
        if (self.imagePosition == QUIButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(QUIButtonImagePosition)imagePosition {
...
//            [self.highlightedBackgroundLayer q_removeDefaultAnimations]; 这一段不明白QUI的意思
...
- (void)setCornerRadius:(CGFloat)cornerRadius {
    ...
    if (cornerRadius != QUIButtonCornerRadiusAdjustsBounds) {
