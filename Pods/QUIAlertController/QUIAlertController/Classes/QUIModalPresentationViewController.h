...
@class QUIModalPresentationViewController;
@class QUIModalPresentationWindow;

typedef NS_ENUM(NSUInteger, QUIModalPresentationAnimationStyle) {
    QUIModalPresentationAnimationStyleFade,    // 渐现渐隐，默认
    QUIModalPresentationAnimationStylePopup,   // 从中心点弹出
    QUIModalPresentationAnimationStyleSlide    // 从下往上升起
};

@protocol QUIModalPresentationContentViewControllerProtocol <NSObject>
...
@protocol QUIModalPresentationViewControllerDelegate <NSObject>
 ...
 *  @param  controller  当前的modalController
 */
- (void)willHideModalPresentationViewController:(QUIModalPresentationViewController *)controller;
 ...
 *  @param  controller  当前的modalController
 */
- (void)didHideModalPresentationViewController:(QUIModalPresentationViewController *)controller;
 ...
 *  支持 3 种方式显示浮层：
 *
 *  1. **推荐** 新起一个 `UIWindow` 盖在当前界面上，将 `QUIModalPresentationViewController` 以 `rootViewController` 的形式显示出来，可通过 `supportedOrientationMask` 支持横竖屏，不支持在浮层不消失的情况下做界面切换（因为 window 会把背后的 controller 盖住，看不到界面切换）。
 ...
 *  @endcode
 *
 *  3. 将浮层作为一个 subview 添加到 `superview` 上，从而能够实现在浮层不消失的情况下进行界面切换，但需要 `superview` 自行管理浮层的大小和横竖屏旋转，而且 `QUIModalPresentationViewController` 不能用局部变量来保存，会在显示后被释放，需要自行 retain。横竖屏跟随当前界面的设置。
 ...
 *
 *  @see QUIAlertController
 *  @see QUIDialogViewController
 *  @see QUIMoreOperationController
 */
@interface QUIModalPresentationViewController : UIViewController

@property(nullable, nonatomic, weak) IBOutlet id<QUIModalPresentationViewControllerDelegate> delegate;
 ...
 */
@property(nullable, nonatomic, strong) IBOutlet UIViewController<QUIModalPresentationContentViewControllerProtocol> *contentViewController;
 ...
 *
 *  `QUIModalPresentationViewController`会自动给自定义的`dimmingView`添加手势以实现点击遮罩隐藏浮层。
 */
...
/**
 *  设置要使用的显示/隐藏动画的类型，默认为`QUIModalPresentationAnimationStyleFade`。
 *  @warning 当使用了`showingAnimation`和`hidingAnimation`时，该属性无效
 */
@property(nonatomic, assign) QUIModalPresentationAnimationStyle animationStyle UI_APPEARANCE_SELECTOR;
...
/// 是否包含TextField，如果没有，就不初始化KeyboardManager
@property(nonatomic, assign) BOOL containsTextField;

/**
 *  管理自定义的浮层布局，将会在浮层显示前、控件的容器大小发生变化时（例如横竖屏、来电状态栏）被调用，请在 block 内主动为 view 设置期望的 frame，设置时建议用 q_frameApplyTransform 取代 setFrame:，否则在有键盘的情况下，显隐动画可能有错。
...
/**
 *  如果你有一个控件，内部通过 QUIModalPresentationViewController 实现显隐功能，那么这个控件建议实现这个协议，这样当 + [QUIModalPresentationViewController hideAllVisibleModalPresentationViewControllerIfCan] 被调用的时候，可以通过 hideModalPresentationComponent 来隐藏你的控件，否则会直接调用 QUIModalPresentationViewController 的 hide 方法，那样可能导致你的控件无法正确被隐藏。
 */
@protocol QUIModalPresentationComponentProtocol <NSObject>
...
@interface QUIModalPresentationViewController (Manager)
 ...
 *  @see    QUIModalPresentationComponentProtocol
 *  @warning 当要隐藏一个 modalPresentationViewController 时，如果这个 modal 有实现 QUIModalPresentationComponentProtocol 协议，则会调用它的 hideModalPresentationComponent 方法来隐藏，否则直接用 QUIModalPresentationViewController 的 hideWithAnimated:completion:
 */
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan;
@end

/// 专用于QUIModalPresentationViewController的UIWindow，这样才能在`UIApplication.sharedApplication.windows`里方便地区分出来
@interface QUIModalPresentationWindow : UIWindow
